...
\begin{document}
    ...
    \begin{tabular}{ |m{1.3cm}|m{1.3cm}|m{3.3cm}|m{2.7cm}|>{\raggedleft\arraybackslash}m{2cm}| }
        ...
        \hline
        \hline
        \texttt{0x0008} & \texttt{0x01E7} & init SIO/2 & \textbf{\multirow{3}{4em}{BIOS}} & 480 \\
        \texttt{0x01E8} & \texttt{0x133F} & BIOS code & & 4,448 \\
        ...
        \hline
        \texttt{0x2780} & \texttt{0x3D5F} & CLI code & \textbf{CLI} & 5,600 \\
        \hline
        \texttt{0x3B60} & \texttt{0x3E5F} & Bootstrap & \textbf{BOOTSTRAP} & 256 \\
        \hline
        \texttt{0x3E60} & \texttt{0x3FFF} & Free & & 416 \\
        \hline
    \end{tabular}
    ...
    \begin{longtable}{ |l|l|l|l|l| }\hline
        ...
        & \multicolumn{2}{|l|}{\textbf{System Variables}} & 341\\
        \hline
        \texttt{0x4180} & \texttt{0x421F} 
        & \multicolumn{2}{|l|}{\textbf{Reserved for future use}} & 160\\
        \hline
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \texttt{0x40CE} - \textbf{CLI\_buffer} (6 bytes): generic
            buffer.
            \item \texttt{0x40D4} - \textbf{CLI\_buffer\_cmd} (16 bytes): when a
            user enters a command and its parameters, the command alone is
            stored here.
            \item \texttt{0x40E4} - \textbf{CLI\_buffer\_parm1\_val} (16 bytes):
            when a user enters a command and its parameters, the first parameter
            is stored here.
            \item \texttt{0x40F4} - \textbf{CLI\_buffer\_parm2\_val} (16 bytes):
            when a user enters a command and its parameters, the second parameter
            is stored here.
            \item \texttt{0x40F4} - \textbf{CLI\_buffer\_pgm} (32 bytes): generic
            buffer.
            \item \texttt{0x40F4} - \textbf{CLI\_buffer\_full\_cmd} (64 bytes):
        ...
        \item \textbf{RTC}
        \begin{itemize}
            \item \texttt{0x4164} - \textbf{RTC\_hour} (1 byte): 24h format,
            in hexadecimal (\texttt{0x00}-\texttt{0x17}).
            \item \texttt{0x4165} - \textbf{RTC\_minutes} (1 byte): in
            hexadecimal (\texttt{0x00}-\texttt{0x3B}).
            \item \texttt{0x4166} - \textbf{RTC\_seconds} (1 byte): in
            hexadecimal (\texttt{0x00}-\texttt{0x3B}).
            \item \texttt{0x4167} - \textbf{RTC\_century} (1 byte): 20 part of
            year 20xx, in hexadecimal (\texttt{0x14} = 20).
            \item \texttt{0x4168} - \textbf{RTC\_year} (1 byte): xx part of
            year 20xx, in hexadecimal (e.g. \texttt{0x16} = 22). The \textbf{RTC}
            supports until 2079, therefore maximum value is \texttt{0x4F}.
            \item \texttt{0x4169} - \textbf{RTC\_year4} (2 bytes): four digit
            year, in hexadecimal (e.g. \texttt{0x07E6} = 2022). The \textbf{RTC}
            supports until 2079, therefore maximum value is \texttt{0x081F}.
            \item \texttt{0x416B} - \textbf{RTC\_month} (1 byte): in
            hexadecimal (\texttt{0x00}-\texttt{0x0C}).
            \item \texttt{0x416C} - \textbf{RTC\_day} (1 byte): in hexadecimal
            (\texttt{0x00}-\texttt{0x1F}).
            \item \texttt{0x416D} - \textbf{RTC\_day\_of\_the\_week} (1 byte):
        ...
        \item \textbf{Math}
        \begin{itemize}
            \item \texttt{0x416E} - \texttt{MATH\_CRC} (2 bytes): CRC-16 CRC.
            \item \texttt{0x4170} - \texttt{MATH\_polynomial} (2 bytes): CRC-16
            Polynomial.
        \end{itemize}
        \item \textbf{Generic}
        \begin{itemize}
            \item \texttt{0x4172} - \textbf{SD\_images\_num} (1 byte): number
            of Disk Image Files found by \textbf{ASMDC}.
            \item \texttt{0x4173} - \textbf{DISK\_current} (1 byte): current 
            \textbf{DISK} unit active. All disk operations will be on this
            \textbf{DISK}.
            \item \texttt{0x4174} - \textbf{DISK\_status} (1 byte): status of
            the \textbf{FDD}.
            \begin{itemize}
            ...
            \end{itemize}
            \item \texttt{0x4175} - \textbf{DISK\_file\_type} (1 byte): File
            Type when creating (\textit{save}) next file.
            \item \texttt{0x4176} - \textbf{DISK\_loadsave\_addr} (2 bytes): see
            \hyperref[sec:howto_readdata]{Read data from DISK} and 
            \hyperref[sec:howto_writedata]{Write data to DISK}.
            \item \texttt{0x4178} - \textbf{tmp\_addr1} (2 bytes): temporary
            storage for an address.
            \item \texttt{0x417A} - \textbf{tmp\_addr2} (2 bytes): temporary
            storage for an address.
            \item \texttt{0x417C} - \textbf{tmp\_addr3} (2 bytes): temporary
            storage for an address.
            \item \texttt{0x417E} - \textbf{tmp\_byte} (1 byte):  temporary
            storage for a byte.
            \item \texttt{0x417F} - \textbf{tmp\_byte2} (1 byte):  temporary
            storage for a byte.
        \end{itemize}
    \end{itemize}
