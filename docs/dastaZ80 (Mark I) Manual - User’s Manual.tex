...
\begin{document}
    ...
    \hspace{1cm}{\LARGE dastaZ80 Appendixes}
    ...
    \begin{itemize}
        ...
        \item Storage device
        \begin{itemize}
            \item CompactFlash Card, formatted with DZFS\footnote{DZFS (dastaZ80
            File System) is a file system of my own design, for mass storage
            devices, aimed at simplicity.}
        \end{itemize}
        \item Video output
        \begin{itemize}
            \item VGA 80 column by 25 lines and 16 colours
        \end{itemize}
        \item Keyboard
        \begin{itemize}
            \item Acorn Archimedes A3010 keyboard. 102 keys with 12 function
            keys, cursor keys and keypad
        \end{itemize}
        \item Case
        \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item A CompactFlash card.
        \item A VGA monitor.
    \end{itemize}

    \begin{enumerate}
        \item Introduce the CompactFlash card in the CF slot at the back of the
    ...
    CompactFlash (CF) card in the case of dastaZ80, and allows the OS to load
    and save data in the CF card. From now on, referred as \textbf{DISK}.
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item No support for extensions.
        \end{itemize}
    ...
    in the screenshot). The BIOS, Kernel and CLI versions and some processes
    like detecting \textbf{RAM} and initialising the \textbf{DISK}.

    After that information, you’ll see the symbol $>$

    This symbol is called the \textit{command prompt}. To the right of the
    command prompt you will see a flashing underscore. This is called the
    \textit{cursor}. The cursor shows where the command you type will appear.
    ...
    % ==========================================================================
        ...
        \subsubsection{halt}
        % ======================================================================
        Disables interrupts and puts the CPU in halted state, effectively
        making the computer unusable until next reboot. SHOULD be used
        before switching the computer off, to ensure no changes are
        happening. MUST not be used while the busy light of the DISK is
        on.
    ...
    %     \subsubsection{date}
    %     % ======================================================================
    %     Shows the current date from the Real-Time Clock (\textbf{RTC}).
    ...
    %     \hspace{1cm}\texttt{Today:\ 12/08/2022}
    ...
    % ==========================================================================
        ...
        \resizebox{12cm}{!}{
            \begin{tabular}{l l l l l}
                \multicolumn{5}{l}{Disk Catalogue}\\
                \multicolumn{5}{l}{-------------------------------------------------------------------------------}\\
                File & Created & Modified & Size & Attributes\\
                \multicolumn{5}{l}{-------------------------------------------------------------------------------}\\
                HelloWorld & 12-03-2022 13:21:44 & 12-03-2022 13:21:22 & 38 &
                E\\
                file2 & 11-05-2022 17:12:45 & 11-05-2022 18:21:32 & 512 & SE
            \end{tabular}
        }}
        ...
        % ======================================================================
        Formats a \textbf{DISK} with DZFS format. This is a destructive action
        and all data in the \textbf{DISK} will be destroyed and make it
        unreadable by other computers that do not support DZFS.
        ...
        \hspace{1cm}\textbf{\textit{partitions}}: currently DZFSV1 only supports
        one partition, so this number MUST be a 1.

        \textbf{Example}: \texttt{$>$ formatdsk mainDisk,1}

        Will format the CompactFlash card inserted in the CF slot at the back of
        the computer case with one partition, having \textit{mainDisk} as disk
        label.
        ...
        character of the filename to ~, which makes it to not show up with the
        command \textit{cat}. Hence, it can be undeleted by simply renaming the
        file. But be aware, when saving new files DZFS looks for a free space
        \footnote{By free space on the \textbf{DISK} we understand a Block in
        the DZFS BAT that was never used before by a file.} on the
        \textbf{DISK}, but if it does not find any it starts re-using space from
        files marked as deleted and hence overwriting data on the \textbf{DISK}.
        ...
        \resizebox{12cm}{!}{
            \begin{tabular}{l r l l}
                \multicolumn{4}{l}{CompactFlash Information}\\
