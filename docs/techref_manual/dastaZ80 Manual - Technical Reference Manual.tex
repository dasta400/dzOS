...
\begin{document}
    ...
    GND), the ROM select signal (\textit{/ROM\_CE}) from the Control Bus, and a
    special signal (\textit{EXTROM}) to indicate that a cartridge is inserted.
    ...
    signals are buffered via 74HCT244 Octal Buffers and Line Drivers.
        ...
        The insertion/extraction of any cartridge SHOULD be done with the
        computer switched off, to avoid possible damages, or at least weird
        behaviour.

        As the internal \textbf{ROM} is disabled when a cartridge is inserted,
        the software on the cartridge ROM won't have access to the BIOS and
