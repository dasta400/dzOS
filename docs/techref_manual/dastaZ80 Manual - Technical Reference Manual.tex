...
\begin{document}
    ...
    \textbf{How disk images work?}

    An SD card, formatted with FAT32 format, contains one or many so called
    \textit{Disk Images}. These disk images are bit-by-bit binary
    representations of a storage device structure. In our case, a dastaZ80
    compatible disk (i.e. a disk formatted with DFZFS).

    At boot, the \textbf{ASMDC} opens (\textit{fopen()}) all disk images listed
    in the file \textit{\_disks.cfg} (also contained in the SD card) and adds
    the pointers returned by \textit{fopen()} to an array. Everytime DZOS wants
    to read a file, it sends the byte \texttt{0xB2}, to read a sector from the
    image, followed by the disk image number from which to read. DZOS reads
    files sector (512 bytes) by sector.

    When a \textit{halt} command is issued by the user, \textbf{ASMDC} closes
    all images that opened at boot.
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item I've built a wooden box inside which I've put the boards and
            connectors. Also included a control panel with an LCD screen, from
            which some options can be selected (ROM start address,
            Internal/External HDD, Serial/VGA output). It has turned to be much
            more handy than the all-in-one Acorn Archimedes A3010 I had, because
            1) occupies much less physical space, and 2) allows me easy access
            to the boards for modifications and tests.
        \end{itemize}
        \item \textbf{December 2023}
        \begin{itemize}
            \item I've been doing some modifications to the MS BASIC.
            \item And created a program in C that emulates the SD Card reader
            of the \textbf{ASMDC}, and it can be connected via serial with an
            FTDI-to-USB cable. The speed is 3.8 times faster than the
            \textbf{ASMDC}.
        \end{itemize}
    \end{itemize}
